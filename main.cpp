...
// Luu tru thong tin mot tu: tu, tu loai, nghia, 
struct AWORD {
	string tu;
	string loai;
...
// Luu tru mot tu theo dang doubly-linkist
struct NODEWORD {
	AWORD info;
	NODEWORD *left;
	NODEWORD *right;
...
string TEN_UNG_DUNG = "TU DIEN ANH-VIET";
...
// Khi chèn một ví dụ mới vào đầu danh sách, ví dụ đó sẽ trở thành phần tử đầu tiên trong danh sách.
// trường hợp ta muốn truy cập nhanh chóng và hiệu quả vào ví dụ gần nhất.
...
// Việc chèn từ mới vào cuối danh sách đồng nghĩa với việc từ mới sẽ được xem xét sau các từ có sẵn trong danh sách.
// giúp duy trì thứ tự từ điển và đảm bảo các từ được xử lý theo thứ tự chúng được chèn vào danh sách.
void chenTuVaoCuoiTd(hashtable *&tudien, NODEWORD *p) {
	...
	if (tudien[i].head == NULL) {
		...
		// ta sử dụng con trỏ right để thêm nút mới vào cuối danh sách và cập nhật liên kết giữa các nút, 
		// trong khi con trỏ left được sử dụng để truy cập ngược từ nút cuối cùng đến nút trước đó
...
// hash function
// Ký tự đầu tiên của từ chuyển đổi thành mã ASCII và trừ đi giá trị ASCII của ký tự 'a' (97) để tính toán vị trí tương ứng trong bảng chữ cái.
// Ket qua tra ve dai dien cho tap tu dien ma tu thuoc ve
...
// chuyen du lieu tung dong thanh cau truc du lieu (NODEWORD)
